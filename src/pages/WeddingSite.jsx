...
function WeddingCrest({ compact = false }) {
  return <div className={`wedding-crest ${compact ? 'compact' : ''}`} aria-label="Monograma Allyson e Mayara, 31 de outubro de 2026"><span>A</span><i>&</i><span>M</span><small>31 · 10 · 2026</small></div>;
}
...
function GiftIllustration({ item }) {
  ...
  const icons = { cozinha: CookingPot, eletro: WashingMachine, mesa: UtensilsCrossed, casa: House, viagem: Plane };
...
export default function WeddingSite() {
  ...
  const openGift = (item, interactionId) => {
    setGift(item);
    setGiftAmount(item.price);
    setGiftReference(`${item.id}-${Math.round(interactionId)}`);
  };
