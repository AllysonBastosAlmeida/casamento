...
export default function WeddingAdmin() {
  ...
  const syncExcel = useCallback(async () => { setLoading(true); setData(current => ({ ...current, rsvps: [] })); setExcelStatus({ connected: false, error: '' }); try { const rsvps = mergeRsvps(await loadExcelRsvps()); setData(current => ({ ...current, rsvps })); setExcelStatus({ connected: true, error: '' }); } catch (error) { setExcelStatus({ connected: false, error: error.message || 'Falha ao conectar ao Excel.' }); } finally { setLoading(false); } }, []);
  useEffect(() => {
